...
// GET request - check if admin or public
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    
    // Check if this is an admin request (requires auth)
    $isAdminRequest = isset($_GET['admin']) || isset($_GET['status']) || isset($_GET['category']);
    ...
    if ($isAdminRequest) {
        ...
        // Public endpoint - no auth required, only show active listings
        try {
            $pdo = getDbConnection();
            ...
            // Parse JSON fields and format data with camelCase transformation
            foreach ($listings as &$listing) {
                $listing['is_featured'] = (bool)$listing['is_featured'];
                $listing['price'] = floatval($listing['price']);
                $listing['bedrooms'] = intval($listing['bedrooms']);
